...
import { Home, LogOut, UserPlus, Menu, X } from 'lucide-react';
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const menuItems = [
    { name: 'Home', icon: Home, path: '/enrollment/home' }
  ];
  ...
  return (
    <>
      ...
      >
        {/* Logo Section */}
        <div className="p-4 lg:p-6 border-b border-purple-500">
          <div className="flex items-center justify-center mb-3">
            <div className="bg-white p-2 lg:p-3 rounded-full">
              <UserPlus size={32} className="text-purple-600 lg:w-10 lg:h-10" />
            </div>
          </div>
          <h2 className="text-center text-base lg:text-lg font-bold">Enrollment Portal</h2>
        </div>

        {/* User Info */}
        <div className="p-3 lg:p-4 border-b border-purple-500">
          <div className="text-center">
            <div className="w-12 h-12 lg:w-16 lg:h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-2">
              <span className="text-purple-600 font-bold text-lg lg:text-2xl">
                {user?.name?.charAt(0)}
              </span>
            </div>
            <p className="font-semibold text-sm lg:text-base">{user?.name}</p>
            <p className="text-xs lg:text-sm text-purple-200">Enrollment Officer</p>
            {user?.accountId && (
              <p className="text-xs text-purple-300 mt-1">ID: {user.accountId}</p>
            )}
          </div>
        </div>
        ...
        {/* Logout Button */}
        <div className="p-3 lg:p-4 border-t border-purple-500">
          <button
